...
class RSLRecognitionApp(QMainWindow):
    ...
    def _process_collected_sentence(self):
        ...
        prompt_lines = [
            ...
            "Ты должен взять самый вероятный (стоит первым в списке) жест на каждом шаге и добавить его к предложению.",
            "Ты можешь заменить не более одно самое веротяное слово на одно менее вероятное, если это не выглядит осмысленно в контексте предложения.",
            "Учти, что люди могут показывать не обязательно супер осмысленные предложения, поэтому старайся всегда брать самое вероятное слово на каждом шаге.",
            "Избегай повторения одного и того же жеста подряд, если это не выглядит осмысленно в контексте предложения.",
            "Никогда не добавляй несколько слов из одного шага в предложение, тебе нужно выбрать только одно слово на каждом шаге (предпочтительно самое вероятное).",
            "В качестве ответа выдай одно предложение без размышлений и комментариев.",
            "Финальное предложение должно быть на русском языке и написано без грамматических ошибок, то есть звучать как настоящее предложение. Не осталвляй слова как есть, всегда встраивай их в складное предложение, можешь заменять слова на синонисы чтобы добиться более логческого связного предложения.",
            "Пример: если входное предложение содержит слова 'набояться' 'слона', нужно написать 'Я боюсь слона'",
